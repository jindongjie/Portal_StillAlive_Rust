...
static PRINT_LOCK: once_cell::sync::Lazy<Arc<Mutex<()>>> =
    once_cell::sync::Lazy::new(|| Arc::new(Mutex::new(())));

static CURSOR_POSITION: once_cell::sync::Lazy<Mutex<(i32, i32)>> =
    once_cell::sync::Lazy::new(|| Mutex::new((1, 1)));

static IS_DRAW_END: once_cell::sync::Lazy<Mutex<bool>> =
...
fn get_ascii_art_x() -> usize {
    get_lyric_width()
        .saturating_add(4)
        .saturating_add((get_credits_width().saturating_sub(ASCII_ART_WIDTH)) / 2)
}
...
impl ThreadCredits {
    fn run(&self) {
        ...
        for ch in CREDITS.chars() {
            ...
            if ch == '\n' {
                ...
                if last_credits.len() > get_credits_height() {
                    last_credits =
                        last_credits.split_off(last_credits.len() - get_credits_height());
                }
            }
            let _lock = PRINT_LOCK.lock().unwrap();
        ...
        move_cursor(
            CURSOR_POSITION.lock().unwrap().0,
            CURSOR_POSITION.lock().unwrap().1,
            false,
            false,
        );
        credit_x += 1;
        ...
        while LYRICS[current_lyric].mode != 9 {
            ...
            if current_time > LYRICS[current_lyric].time as f32 {
                ...
                let interval = if LYRICS[current_lyric].interval < 0.0 {
                    (LYRICS[current_lyric + 1].time - LYRICS[current_lyric].time) as f32
                        / 100f32
                        / word_count
                } else {
                ...
                match LYRICS[current_lyric].mode {
                    ...
                    2 => {
                        draw_aa(
                            ASCII_ART_HEIGHT,
                            ASCII_ART_WIDTH,
                            (&LYRICS[current_lyric].words).parse().unwrap(),
                            &[],
                        );
